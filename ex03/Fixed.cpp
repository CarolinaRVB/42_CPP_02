...
/*   Updated: 2024/08/12 23:15:05 by crebelo-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
	Calling absolute on the rawbits to convert any negative areas to positive,
	then we shift the bits according to the mbits to scale the absolute integer
	back to it's floating-point format which is used in the Fixed float constructor
*/
Fixed Fixed::abs() const {
    return Fixed(std::abs(_mn) / float(1 << _mbits));
}
